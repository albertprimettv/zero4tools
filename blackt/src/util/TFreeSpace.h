...
public:
   ...
   * @param rangeMustContainWholeBlock If true, and start/end are not -1,
   *   a block of space must fall entirely within those boundaries to be
   *   successfully claimed. If false, only the starting point needs to be
  ...
  int claim(int size, int start = -1, int end = -1,
    bool rangeMustContainWholeBlock = true);
