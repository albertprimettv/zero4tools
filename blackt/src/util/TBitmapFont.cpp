...
void TBitmapFont::load(std::string inprefix) {
  ...
  glyphSymbolTable = TThingyTable();
  // this table is supposed to be in UTF8 format.
  // however, legacy fonts may use some other encoding.
  // those fonts will not use any of the new features that rely on this table,
  // so if we can't read the table as UTF8, we simply ignore it and go on.
  try {
    glyphSymbolTable.readUtf8(inprefix + "table.tbl");
  }
...
/*  for (int k = 0; k < numChars; k++) {
  ...
  for (auto it = index.begin(); it != index.end(); it++) {
    ...
    // old format
    if (it->first.substr(0, 4).compare("char") == 0) {
      sectionName = it->first;
    }
    // new format
    else if (it->first.substr(0, 8).compare("symchar_") == 0) {
      sectionName = it->first;
    }
    else continue;
  ...
  for (int k = 0; k < numChars; k++) {
    ...
    // if a character definition in the new format exists, use that
    if (glyphSymbolTable.hasEntry(k)) {
      std::string possibleSectionName
        = std::string("symchar_") + glyphSymbolTable.getEntry(k);
      if (index.hasSection(possibleSectionName)) {
//        std::cerr << k << " " << possibleSectionName << std::endl;
        sectionName = possibleSectionName;
      }
    }
    ...
    if ((fixedWidth >= 0) && (data.advanceWidth < fixedWidth)) {
...
//  KerningMatrix tuneMatrix(numChars, numChars);
  for (auto it = index.begin(); it != index.end(); it++) {
    // old format
    if (it->first.substr(0, 16).compare("AutoKerningTune_") == 0) {
      std::string remaining = it->first.substr(16, std::string::npos);
      bool mainCharIsLeft = true;
      if (remaining[0] == 'R') mainCharIsLeft = false;
      remaining = remaining.substr(2, std::string::npos);
      int rightCharId = TStringConversion::stringToInt(
        std::string("0x") + remaining);
        
      for (auto jt = it->second.begin(); jt != it->second.end(); jt++) {
        std::vector<int> targets = getCharList(jt->first);
        int offset = TStringConversion::stringToInt(jt->second);
        for (int i = 0; i < targets.size(); i++) {
          int leftCharId = targets[i];
          
          if (mainCharIsLeft)
            offsetKerning(rightCharId, leftCharId, offset);
          else
            offsetKerning(leftCharId, rightCharId, offset);
        }
        // value = additional offset to apply when the subcharacter is on left
/*        int leftCharId = TStringConversion::stringToInt(jt->first);
        int offset = TStringConversion::stringToInt(jt->second);
//        tuneMatrix.data(rightCharId, leftCharId) = offset;
        
        offsetKerning(leftCharId, rightCharId, offset);*/
      }
    }
    // new format, using symbols instead of raw glyph indices
    else if (it->first.substr(0, 19).compare("AutoKerningTuneSym_") == 0) {
      std::string remaining = it->first.substr(19, std::string::npos);
      std::string symbol = "";
      ...
      {
        TBufStream ifs;
        ifs.writeString(remaining);
        ifs.seek(0);
        
        // use glyph symbol table to match symbol from string
        TThingyTable::MatchResult result
          = glyphSymbolTable.matchTableEntry(ifs);
        if (result.id == -1) {
          throw TGenericException(T_SRCANDLINE,
                                  "TBitmapFont::load()",
                                  "Tried to tune kerning of unknown symbol");
        }
        
        rightCharId = result.id;
        
        // symbol must be immediately followed by an underscore
        TParse::matchChar(ifs, '_');
        
        // now fetch target side
        if (tolower(ifs.peek()) == 'r') mainCharIsLeft = false;
      }
        
      for (auto jt = it->second.begin(); jt != it->second.end(); jt++) {
        std::vector<int> targets = getSymbolizedCharList(jt->second);
        int offset = TStringConversion::stringToInt(jt->first);
        for (int i = 0; i < targets.size(); i++) {
          int leftCharId = targets[i];
...
const TBitmapFont::KerningMatrix& TBitmapFont::getKerningMatrix() const {
  return kerningMatrix;
}

const TThingyTable& TBitmapFont::getGlyphSymbolTable() const {
  return glyphSymbolTable;
}
...
int TBitmapFont::computeKerning(
  ...
  // FIXME: this does not have intuitive behavior in the case where second is
  // wider than first, and second WOULD collide with first if we pushed it far
  // enough to the left, but first is too narrow for those columns to ever
  // get checked (because we stop pushing left once second's left side reaches
  // the left boundary of first).
  // this results in a detection of "no collision" and a resultant kerning of
  // zero by default, which is not what anyone would expect from looking at the
  // two characters.
  // the workaround is to manually add dummy padding to the left side of first,
  // but that's fiddly and awkward.
...
std::vector<int> TBitmapFont::getCharList(std::string charListStr) {
  std::vector<int> result;
  
  int pos = 0;
  while (pos < charListStr.size()) {
    std::string next = "0x";
    while ((pos < charListStr.size()) && (charListStr[pos] != '/')) {
      next += charListStr[pos++];
    }
    
    result.push_back(TStringConversion::stringToInt(next));
    
    // skip "/" (or go one past end of string)
    ++pos;
  }
...
std::vector<int> TBitmapFont::getSymbolizedCharList(std::string charListStr) {
  std::vector<int> result;
  
  TBufStream ifs;
  ifs.writeString(charListStr);
  ifs.seek(0);
  ...
  while (!ifs.eof()) {
    ...
    if (matchResult.id == -1) {
      throw TGenericException(T_SRCANDLINE,
                              "TBitmapFont::getSymbolizedCharList()",
                              "Unknown symbol in tuning list");
    }
    
    result.push_back(matchResult.id);
    
    // symbols are slash-separated
    if (TParse::checkChar(ifs, '/')) TParse::matchChar(ifs, '/');
  }
