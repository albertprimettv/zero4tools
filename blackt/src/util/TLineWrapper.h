...
//	virtual bool isRegionalized() const;
  ...
  // set a flag indicating that web-insecure features should be disabled.
  // TODO: would be better to tie this to a compile-time flag so that the
  // insecure functionality just doesn't exist in the server builds
  void setSecureModeOn(bool secureModeOn__);
  void setLocaleId(std::string localeId__);
  
  std::string getCurrentScriptBufferContents() const;
  std::string getCurrentWordBufferContents() const;
  
  void setXSize(int xSize__);
  void setYSize(int ySize__);
protected:
...
//  bool checkSymbol(BlackT::TStream& ifs, std::string& symbol);
  ...
  void processLoadRegionalTable(BlackT::TStream& ifs);
