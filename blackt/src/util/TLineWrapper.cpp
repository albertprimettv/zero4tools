...
//    if (onlySpace) continue;
  ...
  // content
  if ((currentWordBuffer.size() > 0) || (currentScriptBuffer.size() > 0)) {
    flushActiveScript();
  }
}
...
//bool TLineWrapper::isRegionalized() const {
//	return false;
//}
...
void TLineWrapper::setSecureModeOn(bool secureModeOn__) {
  secureModeOn = secureModeOn__;
}

void TLineWrapper::setLocaleId(std::string localeId__) {
	localeId = localeId__;
}

std::string TLineWrapper::getCurrentScriptBufferContents() const {
  return std::string(currentScriptBuffer.data().data(),
    currentScriptBuffer.size());
}

std::string TLineWrapper::getCurrentWordBufferContents() const {
  return std::string(currentWordBuffer.data().data(),
    currentWordBuffer.size());
}

void TLineWrapper::setXSize(int xSize__) {
  xSize = xSize__;
}

void TLineWrapper::setYSize(int ySize__) {
  ySize = ySize__;
}
...
TLineWrapper::Symbol TLineWrapper::fetchNextSymbol(BlackT::TStream& ifs) {
  if (ifs.eof()) {
    throw TGenericException(T_SRCANDLINE,
                            "TLineWrapper::fetchNextSymbol()",
                            "Line "
                              + TStringConversion::intToString(lineNum)
                              + ":\n  fetchNextSymbol() on EOF stream");
  }
...
void TLineWrapper::processDirective(BlackT::TStream& ifs) {
  ...
  else if (name.compare("LOADREGIONALTABLE") == 0) {
    if (!secureModeOn) {
      processLoadRegionalTable(ifs);
    }
...
//  TThingyTable table(tableName);
  TThingyTable table;
  table.readUtf8(tableName);
  loadThingy(table);
}

void TLineWrapper::processLoadRegionalTable(BlackT::TStream& ifs) {
  std::string baseName = TParse::matchString(ifs);
  TParse::matchChar(ifs, ',');
  std::string tableName = TParse::matchString(ifs);
//  TThingyTable table(baseName + localeId + "/" + tableName);
  TThingyTable table;
  table.readUtf8(baseName + localeId + "/" + tableName);
  loadThingy(table);
  
//  for (int i = 0; i < 256; i++) {
//    if (table.hasEntry(i)) {
//      std::cerr << std::hex << i << ": " << table.getEntry(i) << std::endl;
//    }
//  }
}
