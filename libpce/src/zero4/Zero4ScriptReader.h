#ifndef ZERO4SCRIPTREADER_H
#define ZERO4SCRIPTREADER_H
...
//  typedef std::map<int, int> CharSizeTable;
  ...
  Zero4ScriptReader(BlackT::TStream& src__,
...
//  bool sceneModeOn;
  ...
  std::map<std::string,std::string> fontNameToOutputTableName;
  std::map<std::string,BlackT::TThingyTable> outputTableNameToContent;
...
//  bool checkSymbol(BlackT::TStream& ifs, std::string& symbol);
  ...
  void processStartMetaRegion(BlackT::TStream& ifs);
