...
int main(int argc, char* argv[]) {
  ...
  if (script.hasKey("Properties", "background")) {
    backgroundFileName = script.valueOfKey("Properties", "background");
  }
...
//    TOfstream ofs(destName.c_str(), ios_base::binary);
    ...
    if (!backgroundFileName.empty()) {
      ofs.open(backgroundFileName.c_str());
    }
...
//        std::cerr << std::hex << it->first << std::endl;
        ofs.seek(it->first * PcePattern::size);
//      }
  ...
  for (SectionKeysMap::const_iterator it = script.cbegin();
    ...
    bool halfFormat = false;
    
    if (script.hasKey(sectionName, "format")) {
      if (script.valueOfKey(sectionName, "format").compare("half") == 0) {
        halfFormat = true;
      }
    }
