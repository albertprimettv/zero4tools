...
//  TOpt::readNumericOpt(argc, argv, "-yoffset", &yOffset);
  ...
  TGraphic grp;
  if (!inPng.empty()) TPngConversion::RGBAPngToGraphic(inPng, grp);
...
/*        if (subCmd.compare("basepattern") == 0) basePattern = param;
    ...
    else if (cmd.compare("openGrp") == 0) {
      ExtPceSpriteId sprite;
      sprite.cmd = ExtPceSpriteId::cmd_openGrp;
      sprite.inputName = TParse::matchString(ifs);
      sprites.push_back(sprite);
    }
    else {
...
/*      // pad to specified output size
    ...
    else if (it->cmd == ExtPceSpriteId::cmd_openGrp) {
      TPngConversion::RGBAPngToGraphic(it->inputName, grp);
      previewGrp = grp;
      previewGrp.clear(TColor(0, 0, 0));
      continue;
    }
    ...
    TTwoDArray<PceSpritePattern> subtiles(w, h);
...
//        ++currentPatternNum;
        subtiles.data(i, j) = spritePattern;
        
        if (!hasNonemptyTile && (spritePattern != emptyTile))
          hasNonemptyTile = true;
          
/*          for (int j = 0; j < 16; j++) {
            for (int i = 0; i < 16; i++) {
              std::cerr << (int)spritePattern.data_.data(i, j);
            }
            std::cerr << std::endl;
          }
          char c;
    ...
    // ignore totally empty sprites
    // TODO: this should probably be a toggleable option
    if (!hasNonemptyTile) {
      continue;
    }
    ...
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        subtiles.data(i, j).write(grpOfs);
        ++currentPatternNum;
      }
    }
